...
/**
 * Track-2 authentication for headless agents.
 *
 * <p>Resolves a username + password against LDAP, then extracts the tenant entitlement set from
 * the LDAP user record's {@code ou} (or whichever attribute is configured). Spring LDAP is
 * blocking, so the bind happens on {@link Schedulers#boundedElastic()}.</p>
 */
public class LdapBasicAuthenticationManager implements ReactiveAuthenticationManager {
    ...
    public LdapBasicAuthenticationManager(LdapProperties props) {
        this.props = props;
    }
    ...
    private Authentication bind(String username, String password) throws javax.naming.NamingException {
        ...
        // Force a connection to confirm credentials.
        ctx.getContext(ctx.getUserDn(), password).close();

        // Fetch tenant entitlements via manager bind.
        ...
        List<List<String>> tenants = template.search(
                ...
                new TenantAttributeMapper(props.getTenantAttribute()));
        ...
        // Technical / headless principals are writers by definition — they exist to push
        // schemas and entries from build pipelines. Audit trail captures who exactly did it.
    ...
    private static final class TenantAttributeMapper implements AttributesMapper<List<String>> {
        private final String name;
        TenantAttributeMapper(String name) { this.name = name; }
