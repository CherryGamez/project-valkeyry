...
```powershell
$env:VALKEYRY_ADMIN_USERNAME = "admin"
$env:VALKEYRY_ADMIN_PASSWORD = "change-me-please"
$env:VALKEYRY_JWT_SECRET     = "a-32-byte-or-longer-random-string!!!"
```
...
```powershell
$env:VALKEYRY_SSO_ENABLED   = "false"   # default off
$env:VALKEYRY_LDAP_ENABLED  = "false"   # default off
# When SSO is on, also set:
$env:VALKEYRY_OIDC_ISSUER   = "http://localhost:8079/default"
```
...
Spin up the full hybrid stack (Postgres + OpenLDAP + Keycloak + the app) with the
`hybrid` Compose profile:

```powershell
cd C:\projects\valkeyry-ecosystem\valkeyry-config
...
```powershell
# Direct LDAP login through /api/v1/auth/login (this hits AuthService → LdapBasicAuthenticationManager)
...
$bobJwt = $res.token
Invoke-RestMethod -Uri "http://localhost:8081/api/v1/auth/me" `
                  -Headers @{ Authorization = "Bearer $bobJwt" }
# → role=reader (LDAP agents default to reader unless they carry an explicit role)
...
$kc = Invoke-RestMethod -Method Post `
    -Uri "http://localhost:8079/realms/valkeyry/protocol/openid-connect/token" `
    -ContentType "application/x-www-form-urlencoded" `
    -Body @{ grant_type="password"; client_id="valkeyry-config";
...
winget install OpenJS.NodeJS.LTS    # if Node isn't already present
...
cURL flow (`curl.exe` ships with Windows 10+; PowerShell's built-in `curl`
alias points to `Invoke-WebRequest`, so call `curl.exe` explicitly):

```powershell
curl.exe -X POST http://localhost:8081/api/v1/tools/convert/dmn `
         -H "X-API-Key: $env:VALKEYRY_API_KEY" `
         -F "file=@examples\import\loan-approval.dmn" | ConvertFrom-Json | Select-Object -ExpandProperty tables | Select-Object -First 1
```
...
py examples\import\_generate_products_xlsx.py
```
...
```powershell
# Window 1 — start the receiver (Node ≥ 18)
cd valkeyry-config\examples\webhooks
node receiver.js                                # listens on :9099

# Window 2 — register + trigger
$JWT = (curl.exe -s -X POST http://localhost:8081/api/v1/auth/login `
        -H 'Content-Type: application/json' `
        -d '{"username":"admin","password":"admin"}' | ConvertFrom-Json).token

curl.exe -X POST http://localhost:8081/api/v1/tenants/demo-tenant/webhooks `
...
curl.exe -X POST http://localhost:8081/api/v1/tenants/demo-tenant/tables `
         -H 'X-API-Key: plugin-test-key' -H 'Content-Type: application/json' `
         -d '{\"tableName\":\"ping\",\"schema\":{\"type\":\"object\",\"properties\":{\"x\":{\"type\":\"string\"}}}}'
```

Window 1 should print the event headers, body and `Signature valid? : YES`.
Full payload reference: [`examples\webhooks\README.md`](examples/webhooks/README.md).

> **PowerShell quoting tip:** the inner JSON in the `-d` flag uses escaped
> double quotes (`\"`). If you prefer single quotes outside and double quotes
> inside, you can also write the body to a file and use `-d "@payload.json"`.
