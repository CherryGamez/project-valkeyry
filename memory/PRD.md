...
### 2026-02-07 — Examples folder + fix: AuditWebhookPublisher silent NPE in production
**New:** `valkeyry-config/examples/` — fixtures + recipes that round-trip every public
endpoint. Verified end-to-end against the real Spring backend with Newman (26/26 requests,
0 failures) and a local Node receiver (HMAC verified).

```
examples/
├── README.md
├── postman/
│   ├── valkeyry-config.postman_collection.json   # 26 chained requests, 7 folders
│   └── valkeyry-config-local.postman_environment.json
├── import/
│   ├── customers.csv                             # 8 rows × 7 cols
│   ├── products.xlsx                             # 2 sheets, generated by:
│   ├── _generate_products_xlsx.py                # openpyxl re-generator
│   └── loan-approval.dmn                         # Camunda DMN 1.3, FIRST hit policy, 6 rules
└── webhooks/
    ├── README.md                                 # full payload + HMAC reference
    ├── receiver.js                               # Node :9099 receiver, prints HMAC verdict
    └── payload-sample.json                       # exact JSON shape (DECLARE_TABLE example)
```

Both `MAC_TEST_GUIDE.md` and `WINDOWS_TEST_GUIDE.md` got a new §11 referencing the folder.

**Found while testing — fixed:**
`AuditWebhookPublisher` shipped a public no-arg constructor that initialised every field to
`null`. With Spring's "multiple non-annotated constructors → fall back to no-arg" behaviour,
the bean was instantiated via that constructor → `mapper` was null → every audit event hit
`NullPointerException: ObjectMapper.writeValueAsString` and was silently dropped. Static
+ dynamic webhook fan-out was completely broken in production but unit-tests passed because
they construct the publisher directly.

Fix in `valkeyry-config/src/main/java/io/valkeyry/config/service/AuditWebhookPublisher.java`:
- Deleted the all-nulls no-arg constructor.
- Added `@Autowired` on the 3-arg production constructor to break Spring's ambiguity.
- Kept the 2-arg test constructor (delegates to 3-arg with `subscriptions = null`).
- Verified by registering a webhook through the real API and triggering a `DECLARE_TABLE`
  audit — receiver got the POST with `Signature valid? : YES`.

**Bonus front-end fix:** `index.html` (Postman work also surfaced this):
- `ingestBatch()` was POSTing a bare JSON array; backend `BatchIngestRequest` wraps it in
  `{ entries: [...] }` → 400. Wrapped + updated `copyCurlForCurrent()` to match. Verified
  via the Postman §4 *POST /tables/{name}/entries:batch* request → `201 Created`.
